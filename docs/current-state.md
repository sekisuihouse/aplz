...
- `robots.txt`
- 動的 `sitemap.xml`
- サイトマップ全URLのHTTP・metadata・H1・index許可・canonical監査
- Web App Manifest
...
- 全210記事に10件ずつ、合計2,100件の想定検索文を設定
- 近接記事の検索意図を理由・方法・共有・判断・失敗へ分離
- 生成記事の汎用H2を記事主題入りの固有H2へ変換
- 記事別検索文一覧 `strategy/article-search-queries.csv`
- 2,100検索文の不足・完全一致重複を検査する `content:search-map`
- SoftwareApplication JSON-LD
- 困りごと・アプリの動的サイトマップ登録
- 公開コミュニティの動的サイトマップ登録（非公開は除外）
...
| Search Console運用手順 | `docs/search-console-indexing.md` |
| 公開URL監査 | `scripts/audit-indexability.mjs` |
| GEO検証 | `scripts/validate-geo.mjs` |
| GEO実装記録 | `reports/geo-implementation.md` |
| 記事別検索文の生成・検証 | `scripts/generate-article-search-map.mjs` |
| 記事別検索文一覧 | `strategy/article-search-queries.csv` |
| 検索意図実装記録 | `reports/search-intent-implementation.md` |
